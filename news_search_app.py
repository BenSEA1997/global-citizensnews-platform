...
def fetch_google_news(query, cx, api_key, lr=None):
    try:
        ...
        data = resp.json()
        st.write(f"🔍 Google raw results: {len(data.get('items', []))} 筆")  # debug - 看有沒有抓到
...
def fetch_newsdata(query, api_key):
    try:
        ...
        url = "https://newsdata.io/api/1/latest"   # 改用 latest 端點 + 移除 timeframe，最大化抓到伊朗新聞
        ...
        data = resp.json()
        st.write(f"🔍 NewsData raw results: {len(data.get('results', []))} 筆")  # debug - 看有沒有抓到
...
search_query = st.text_input("輸入關鍵字 (檢索新聞)")
        ...
        with st.spinner("正在從多方引擎獲取新聞..."):
            try:
                ...
                if not sorted_news:
                    ...
                    st.success(f"找到 {len(sorted_news)} 筆新聞")
