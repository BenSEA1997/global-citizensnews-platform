...
# ==================== 3. UI 介面 ====================
st.set_page_config(page_title="全球新聞搜尋器 V10.1", layout="wide")
st.title("🌐 全球新聞搜尋器 V10.1")
...
if st.button("執行搜尋", type="primary"):
    ...
    for idx, (s_d, e_d) in enumerate(date_chunks):
        ...
        all_raw_white.extend(fetch_google_news(build_url(query, list(white_list)), "白名單", start_hkt, end_hkt, kw_list))
        all_raw_supp.extend(fetch_google_news(build_url(query), "補充包", start_hkt, end_hkt, kw_list, is_supp=True))
    ...
    # ==================== 4. 診斷面板 (Ver 10.1 強化版) ====================
    with st.expander("🔍 Ver 10.1 深度診斷面板"):
        ...
        st.write(f"**挖掘數據：** 共分 {len(date_chunks)} 段時間抓取 | **放寬策略：** 已啟用關鍵字進取識別")
