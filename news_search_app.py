...
import time  # V14.0 新增: 用於 Serper API 緩衝
import pytz
...
# V14.0 新增: 台灣及中國大陸雜訊黑名單
HK_BLACK_LIST = {"tw.news.yahoo.com", "yahoo.com.tw", "taisounds.com", "newtalk.tw", "udn.com", "storm.mg", "today.line.me", "people.com.cn", "beijing.gov.cn", "cna.com.tw", "i-meihua.com", "ltn.com.tw", "msn.com", "setn.com", "ctinews.com", "worldjournal.com", "cw.com.tw", "tdm.com.mo", "gvm.com.tw", "nownews.com", "youtube.com"}
...
# V14.0 新增: 黑名單及地區後綴檢測邏輯
def check_black(link, source_url, region):
    if region != "香港媒體": 
        return False # 只有選香港媒體時才嚴格過濾
...
def fetch_serper_combined(query, start_date, end_date, gl, hl, white_list, region):
    ...
    # V14.0 修改: Serper News 支援 json=payload, 最大 8 頁, 以及 time.sleep
    ...
    try:
        ...
        for post in res:
            ...
            results.append({
                ...
                "published": dt.strftime("%Y-%m-%d %H:%M"), 
                "platform": "Bluesky", "raw_dt": dt
            })
    except: pass
    return results
 
# ==================== 3. 主介面 UI (V14.0 修正版) ====================
st.set_page_config(page_title="全球 CitizensNews V14.0", layout="wide")
...
if "新聞搜尋" in app_mode:
    ...
    if st.button("搜尋", type="primary"):
        with st.status("🔄 正在搜尋中 ...", expanded=True) as status:
            ...
            # V14.0 修改: 將 region 參數傳入，用於啟動黑名單過濾
    ...
    if st.session_state.news_results is not None and st.session_state.last_news_params == news_params:
        ...
        if not res: st.warning("⚠️ 此關鍵字沒有搜到相關新聞")
            ...
            # V14.0 修改: 增加 icon 到診斷面板
            st.success(f"📊 **資料源診斷**：✅白名單: **{white_count}** ｜ 🔴RSS新聞: **{rss_count}** ｜ 🔵Serper新聞: **{serper_count}** ｜ 🌐補充包: **{supp_count}** ｜ 總數: **{len(res)}**")
            ...
            for n in curr_data:
                # V14.0 修改: 嚴格按照優先級分配圖示
                if n['is_white']:
                    icon = "✅"
                elif n.get('fetch_type') == 'rss':
                    icon = "🔴"
                elif n.get('fetch_type') == 'serper_news':
                    icon = "🔵"
