...
if st.button(search_button):
    if not query:
        ...
        try:
            ...
            # 最簡單條件：只保留語言 + 地區 + size=10（避免 422）
            url_precise = f"https://newsdata.io/api/1/news?apikey={api_key}&q={precise_query}&language=zh,en&country=hk,tw,cn&size=10"
            ...
            # 如果結果少於 5 條，自動切模糊搜尋
            if len(results) < 5:
                url_fuzzy = f"https://newsdata.io/api/1/news?apikey={api_key}&q={query}&language=zh,en&country=hk,tw,cn&size=10"
